...
export function flowLazer(ctx, state, nextStage) {
  ...
  if (intent === INTENT.PHOTO) {
    ...
    if (state.back_text_status === "received" || state.back_text_status === "skipped") {
      if (!state.payment_method) return R("Fotoğrafınızı aldık efendim 😊 Fotoğrafınızda bir sorun olursa ekibimiz bilgi verecektir. Şimdi ödeme tercihinizi iletebilir misiniz? EFT / Havale veya kapıda ödeme şeklinde ilerleyebiliriz.");
      if (state.address_status !== "received") return R(`Fotoğrafınızı aldık efendim 😊 Fotoğrafınızda bir sorun olursa ekibimiz bilgi verecektir.\n\n${TEXT.ORDER_DETAILS}`);
      return R("Fotoğrafınızı aldık efendim 😊 Fotoğrafınızda bir sorun olursa ekibimiz bilgi verecektir.");
    }
    return R("Fotoğrafınızı aldık efendim 😊 Fotoğrafınızda bir sorun olursa ekibimiz bilgi verecektir. Arka yüzüne yazı eklemek ister misiniz? İsterseniz yazıyı buradan iletebilirsiniz, istemezseniz 'yok' yazabilirsiniz.");
  }
