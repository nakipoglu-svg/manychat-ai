...
// Eksik bilgiye göre akıllı adres isteme
function smartAddressPrompt(state) {
  const hasPhone = state.phone_received === "1";
  const hasAddr = state.address_status === "address_only" || state.address_status === "received";
  if (hasAddr && !hasPhone) return R("Cep telefonu numaranızı da paylaşabilir misiniz efendim? 📱");
  if (!hasAddr && hasPhone) return R("Açık adresinizi paylaşabilir misiniz efendim? 📍 (İl, ilçe, mahalle, sokak)");
...
export function catchAll(ctx, state) {
  ...
  if (isConfirm || isEmoji || isShort) {
    ...
    if (stage === STAGE.WAITING_ADDRESS) return smartAddressPrompt(state);
