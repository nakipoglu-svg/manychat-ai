...
export function normalizeProduct(value) {
  ...
  if (["yonca", "yonca kolye", "isimli yonca", "isimli yonca kolye"].includes(v)) return PRODUCT.YONCA;
...
export function isUnsupportedProductVariant(norm = "") {
  const asksBracelet = hasAny(norm, ["bileklik", "bilezik"]);
  const maleSignal = hasAny(norm, [
    "erkek bileklik", "erkek bilezik", "bay bileklik", "bay bilezik",
    "erkek icin bileklik", "erkek için bileklik",
    "babam icin bileklik", "babam için bileklik",
    "esim icin bileklik", "eşim için bileklik",
    "oglum icin bileklik", "oğlum için bileklik",
  ]);
  return asksBracelet && maleSignal;
}

export function detectProductFromText(norm, stage = "") {
  if (isUnsupportedProductVariant(norm)) return PRODUCT.OTHER;

  const selectionStage = !stage || stage === "waiting_product";
  const lazerVariant =
    /\b(resimli+|resmli|resimlii|resimlim|foto[gğ]?rafli|foto[gğ]?raflı|fotografl[iı]|fotoğrafl[iı]|fotolu|foto lu)\b/.test(norm) ||
    /\b(lazer|razer|raxel)\b/.test(norm);
  const kolyeVariant = /\b(kolye|koly[eeyiıu]+|kolyee|kolue|koley|kolyleleri|kolyelerin|kolyenin|kolyeniz|kolyeninz|madalyon)\b/.test(norm);
  if (lazerVariant && (kolyeVariant || hasAny(norm, ["fiyat", "ucret", "ücret", "ne kadar", "istiyorum", "olan", "model", "resimli olan", "fotoğraflı olacak", "fotograflı olacak"]))) {
    return PRODUCT.LAZER;
  }

  const yoncaVariant = /\b(yonca|yonc|yoca|yonja)\b/.test(norm);
  const yoncaQualifier =
    hasAny(norm, ["kolye", "kolyesi", "kolyeye", "model", "yaprak", "isimli", "istiyorum", "almak", "siparis", "sipariş", "fiyat", "ne kadar"]) ||
    /\b(yonca|yonc|yoca|yonja)\s+(kolye|model|istiyorum|fiyat)/.test(norm);
  if (yoncaVariant && (yoncaQualifier || (selectionStage && /^(yonca|yonc|yoca|yonja)$/.test(norm.trim())))) {
    return PRODUCT.YONCA;
  }
  if (hasAny(norm, ["isimli kolye", "isimli model", "isimli kolye istiyorum"])) return PRODUCT.YONCA;
  ...
  if (hasAny(norm, KW.product_yonca)) return PRODUCT.YONCA;
  ...
  if (selectionStage && hasAny(norm, KW.product_other)) return PRODUCT.OTHER;
  return "";
}

export function extractLetters(raw, norm, product, stage) {
  if (product !== PRODUCT.ATAC && product !== PRODUCT.YONCA) return "";
  if (!["", "waiting_letters", "waiting_product"].includes(stage)) return "";
  raw = String(raw || "").trim();
  // Yonca 4 yaprağa kadar isim/sembol alır → biraz daha uzun metne izin ver
  const isYonca = product === PRODUCT.YONCA;
  const maxLen = isYonca ? 40 : 24;
  const maxParts = isYonca ? 5 : 3;
  if (!raw || raw.length > maxLen) return "";
  // Yonca'da "+" sembol ayırıcı olarak kullanılabilir (Ayşe + kalp)
  if (isYonca ? /[?!.:,/]/.test(raw) : /[?!.:,/]/.test(raw)) return "";
  if (!(isYonca ? /^[a-zA-ZçğıöşüÇĞİÖŞÜ0-9\s&+]+$/ : /^[a-zA-ZçğıöşüÇĞİÖŞÜ\s&]+$/).test(raw)) return "";
...
const TYPO_MAP = [
  // Product typo'ları
  [/\bsipar[sş]im\b/gi, "siparişim"],
  [/\bfyt\b/gi, "fiyat"],
  [/mesajlr[ıi]m[ıi]/gi, "mesajlarımı"],
  [/\bsaana\b/gi, "sana"],
  [/\bssorruyoreum\b|\bssoruyoreum\b/gi, "soruyorum"],
  [/\bcxcevao\b|\bcxceva[oı]\b/gi, "cevap"],
  [/\bvertiyyorsun\b|\bvertiyorsun\b/gi, "veriyorsun"],
  [/araçlı|aracli|aracli|araçli/gi, "ataçlı"],
  [/atajlı|ataslı|ataclı|ataşlı/gi, "ataçlı"],
  [/\bata[sş]\s+kolye/gi, "ataç kolye"],
  [/lazer kolye|lazer kolye/gi, "lazer kolye"],
  [/\b(razer|raxel)\b/gi, "lazer"],
  [/\b(lkolye|kolue|koley|kolyee)\b/gi, "kolye"],
  [/\bkolyleleri\b/gi, "kolyeleri"],
  [/\bresimlimkolye\b/gi, "resimli kolye"],
  [/\bresimlii\b/gi, "resimli"],
  [/\bfotografli\b/gi, "fotoğraflı"],
  [/\bfotoğrafli\b/gi, "fotoğraflı"],
