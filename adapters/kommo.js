...
const FID = {
  ...
  // ── İSİM HAFIZASI ──
  // Kommo'daki "Isim Alindi" metin alanı. İsim bir kez alınınca burada saklanır,
  // böylece bot ad soyadı tekrar sormaz.
  name_received: 2050242,
};
...
function stripEmoji(text) {
  ...
  return text
    ...
    // Paragraf araları (boş satır) KORUNUR:
    .replace(/[ \t]{2,}/g, " ")   // sadece yatay boşluğu teke indir (newline'a dokunma)
    .replace(/[ \t]+\n/g, "\n")   // satır sonundaki boşlukları (emoji kalıntısı) temizle
    .replace(/\n[ \t]+/g, "\n")   // satır başındaki boşlukları temizle
    .replace(/\n{3,}/g, "\n\n")   // en fazla bir boş satır bırak
    .trim();
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Raw entity identifiers are needed for both incoming and outgoing events.
    // Keep them above the outgoing branch: otherwise `lid` is referenced before it exists.
      ...
      if (btnLid) {
        ...
        if (isSwitch || isReselect) {
          const priceText = product === "lazer" 
            ? "Resimli Lazer Kolye fiyatımız 649 TL'dir efendim 😊\n\nKapıda ödeme seçeneğinde kargo firmasının tahsil ettiği +50 TL ek ücret uygulanmaktadır. Kapıda ödeme yalnızca nakittir.\n\nSiparişe devam etmek isterseniz fotoğrafınızı buradan gönderebilirsiniz 📷"
            : "Harfli Ataç Kolye + Bileklik Hediye fiyatımız 549 TL'dir efendim 😊\n\nKapıda ödeme seçeneğinde kargo firmasının tahsil ettiği +50 TL ek ücret uygulanmaktadır. Kapıda ödeme yalnızca nakittir.\n\nSiparişe devam etmek isterseniz istediğiniz harfleri yazabilirsiniz ✍️";
    ...
    const result = await processChat({
      message: effectiveText, ...cf, entry_product: cf.ilgilenilen_urun || "", lead_id: lid, contact_id: contactId,
    });
    ...
    const fieldUpdate = {
      ...
      name_received: result.name_received || cf.name_received || "",
...
function calculateConfidence(result) {
  ...
  } else if (result.ilgilenilen_urun === "atac" || result.ilgilenilen_urun === "yonca") {
